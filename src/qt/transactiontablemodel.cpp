...
{
    QString str = PepperscoreUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit);
    if(showUnconfirmed)
    {
...
//        if(index.column() == Amount && rec->type != TransactionRecord::Generated && (rec->credit+rec->debit) > 0)
        if(index.column() == Amount && rec->type != (TransactionRecord::Generated || TransactionRecord::Staked) && (rec->credit+rec->debit) > 0)
        {
