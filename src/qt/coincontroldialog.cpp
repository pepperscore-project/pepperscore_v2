...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = PepperscoreUnits::EPPE;
    ...
    l2->setText(PepperscoreUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(PepperscoreUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(PepperscoreUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));               // Bytes
    l7->setText((fLowOutput ? (fDust ? tr("DUST") : tr("yes")) : tr("no")));        // Low Output / Dust
    l8->setText(PepperscoreUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    
    // turn labels "red"
    l5->setStyleSheet((nBytes >= 10000) ? "color:red;" : "");                       // Bytes >= 10000
    l7->setStyleSheet((fLowOutput) ? "color:red;" : "");                            // Low Output = "yes"
    l8->setStyleSheet((nChange > 0 && nChange < CENT) ? "color:red;" : "");         // Change < 0.01EPPE
        
    // tool tips
    l5->setToolTip(tr("This label turns red, if the transaction size is bigger than 10000 bytes.\n\n This means a fee of at least %1 per kb is required.\n\n Can vary +/- 1 Byte per input.").arg(PepperscoreUnits::formatWithUnit(nDisplayUnit, CENT)));
    l7->setToolTip(tr("This label turns red, if any recipient receives an amount smaller than %1.\n\n This means a fee of at least %2 is required. \n\n Amounts below 0.546 times the minimum relay fee are shown as DUST.").arg(PepperscoreUnits::formatWithUnit(nDisplayUnit, CENT)).arg(PepperscoreUnits::formatWithUnit(nDisplayUnit, CENT)));
    l8->setToolTip(tr("This label turns red, if the change is smaller than %1.\n\n This means a fee of at least %2 is required.").arg(PepperscoreUnits::formatWithUnit(nDisplayUnit, CENT)).arg(PepperscoreUnits::formatWithUnit(nDisplayUnit, CENT)));
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        if (treeMode)
        {
            ...
            for (int i = 0; i < ui->treeWidget->columnCount(); i++)
                itemWalletAddress->setBackground(i, QColor(255, 255, 255));
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            {
                sAddress = CPepperscoreAddress(outputAddress).ToString().c_str();
                
                // if listMode or change => show Pepperscore address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, PepperscoreUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, PepperscoreUnits::format(nDisplayUnit, nSum));
