...
{
    QString ret = QString("pepperscore:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non EPPE unit input in lnReqAmount, we generate the URI with EPPE as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(PepperscoreUnits::format(PepperscoreUnits::EPPE, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
