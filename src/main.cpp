...
int nCoinbaseMaturity = 100;// Blocks to confirm if mining pow pos
int nStakeMinConfirmations =  250; // 250 Minimum Block confirms 
...
int nBestHeight = -1;
static const int64_t nTargetTimespan =  10 * 60; //600 Seconds 10 Minute Adjust Retagret 
...
const string strMessageMagic = "Pepperscore Signed Message:\n";
...
{
    int64_t nSubsidy = 0 * COIN; // Initial subsidy value
    ...
    else if (nHeight >= 2)
    {
        ...
        // Calculate the start and end heights of the current window
        int startHeight = (nHeight / windowSize) * windowSize;

        // Generate a random block height within the window
        srand(startHeight); // Seed the random number generator with the start height of the window
        int randomBlockHeight = startHeight + (rand() % windowSize);

        // Check if the current block height is the randomly chosen height
        if (nHeight == randomBlockHeight)
        {
            nSubsidy = 5 * COIN; // Higher reward at the randomly determined block
        }
        else
        {
            nSubsidy = 0.09375 * COIN; // Regular reward for other blocks in the window
        }
    }
    ...
    {
        nSubsidy = 0;
        LogPrint("END OF POW", "GetProofOfWorkReward(): create=%s nFees=%d\n", FormatMoney(nFees), nFees);
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int nHeight)
{
    ...
    {
        nSubsidy = 0 * COIN;
        LogPrint("END OF POS", "GetProofOfStakeReward(): create=%s nFees=%d\n", FormatMoney(nFees), nFees);
        return nSubsidy + nFees;
    }
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Pepperscore miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfWork())
    {
        ...
        if (vtx[0].GetValueOut() > nReward && vtx[0].GetValueOut() != (5 * COIN + nFees))
